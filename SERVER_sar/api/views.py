...
def asset(request):
    ...
    if request.method == "GET":
        ...
        models.Memory.objects.create(me_num=server_info.get('memory_use').get('data').get('memory_avg'),host_info=hostname_obj)

        now_time=datetime.datetime.now()
        if int(now_time.timestamp()%1000) ==3:
            end_time=now_time-datetime.timedelta(minutes=10)
